...
const sidebarLinks = [
  ...
  { name: "Invoices", to: "/shipper/dashboard/invoices" },
...
export default function SidebarCarrier({ activeSection, theme }: { activeSection: string, theme: string }) {
  ...
  if (activeSection === "home") {
    ...
    renderingLinks = [
      ...
      { name: "Invoices", to: "/carriers/dashboard/invoices" },
    ];
  }

  const themeClasses = {
    sidebar: theme === "dark" ? "bg-gray-800 text-white" : "bg-gray-200 text-black",
    link: theme === "dark" 
    ...
    icon: theme === "dark" ? "text-orange-500" : "text-gray-500",
  };
  
  return (
    <aside
      className={`w-64 shadow-lg h-screen overflow-y-auto py-4 px-3 rounded ${themeClasses.sidebar}`}
      aria-label="Sidebar"
    >
          ...
          <li key={link.name}>
            ...
            >
              <span className="flex items-center">
                {link.name === "Overview" && (
                  <svg
                    className={`h-5 w-5 mr-3 ${themeClasses.icon}`}
